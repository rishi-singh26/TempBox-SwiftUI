...
enum AddressSchemaV4: VersionedSchema {
    ...
    static var models: [any PersistentModel.Type] {
        [Address.self, Folder.self, Message.self]
    }
...
enum AddressSchemaV41: VersionedSchema {
    static var versionIdentifier = Schema.Version(4, 0, 1)
    ...
    class Message: Identifiable {
        ...
        var flagged: Bool?
        var isDeleted: Bool = false
        var isRemovedFromRemote: Bool = false // When a messages is removed from mail.tm server, this flag will be set to true
...
typealias Address = AddressSchemaV41.Address
typealias Folder = AddressSchemaV41.Folder
...
extension Message {
    convenience init(from apiMessage: APIMessage) {
        self.init(
            ...
            remoteId: apiMessage.id,
            accountId: apiMessage.accountId,
            msgid: apiMessage.msgid,
            fromName: apiMessage.from.name,
            fromAddress: apiMessage.from.address,
            to: Self.flatten(emailAddresses: apiMessage.to) ?? "",
            cc: Self.flatten(emailAddresses: apiMessage.cc),
            bcc: Self.flatten(emailAddresses: apiMessage.bcc),
