...
struct AddAddressView: View {
    ...
    @State var controller = AddAddressViewModel()
    ...
    private func createAddress() async {
        ...
        do {
            let account = try await MailTMNetworkService.shared.createAccount(address: controller.getEmail(), password: controller.password)
            let tokenData = try await MailTMNetworkService.shared.authenticate(address: controller.getEmail(), password: controller.password)
            
            await addressStore.addAddress(
              account: account,
              token: tokenData.token,
              password: controller.password,
              name: controller.addressName,
    ...
    private func login() async {
        ...
        do {
            ...
            let tokenData = try await MailTMNetworkService.shared.authenticate(address: controller.getEmail(), password: controller.password)
            let account = try await MailTMNetworkService.shared.fetchAccount(id: tokenData.id, token: tokenData.token)
            await addressStore.addAddress(
              account: account,
              token: tokenData.token,
              password: controller.password,
    ...
    private func isAddressUnique() -> Bool {
        ...
        let (isUnique, isArchived) = addressStore.isAddressUnique(email: controller.getEmail())
        guard isUnique else {
