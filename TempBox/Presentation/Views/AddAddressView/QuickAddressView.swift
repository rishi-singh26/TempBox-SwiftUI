...
struct QuickAddressView: View {
    ...
    private func createAddress() async {
        ...
        do {
            let (account, password) = try await MailTMNetworkService.shared.generateRandomAccount()
            let tokenData = try await MailTMNetworkService.shared.authenticate(address: account.address, password: password)
