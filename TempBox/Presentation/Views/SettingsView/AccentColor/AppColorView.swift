...
struct AppColorView: View {
    ...
    var sortedCustomColors: [AccentColorData] {
        appStore.customColors.sorted { !$0.isSame && $1.isSame }
    }
    ...
    var body: some View {
        List {
            ...
            ColorsListSection(colors: AppStore.defaultAccentColors)
        ...
        .sheet(isPresented: $showAddColorSheet) {
            AddColorView()
                .sheetAppearanceSetup(tint: appStore.accentColor(colorScheme: colorScheme))
        }
    ...
    private func ColorTile(accentColor: AccentColorData, hasActions: Bool) -> some View {
        ...
        let tile = Button {
            ...
            appStore.selectedAccentColorData = accentColor
        } label: {
    ...
    private func ColorTileLabel(accentColor: AccentColorData) -> some View {
        let isSelected = appStore.selectedAccentColorData.id == accentColor.id
    ...
    private func deleteColor(color: AccentColorData) {
        appStore.deleteCustomColor(colorData: color)
    }
}
