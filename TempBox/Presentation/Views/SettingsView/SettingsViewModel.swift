...
class SettingsViewModel {
    var selectedSetting: SettingPage = .importPage
    var navigationStack: [SettingPage] = [.importPage]
    var currentPointer: Int = 0
    var isNavigatingManually = true


    // MARK: - Import Page properties
    var isPickingFile: Bool = false
    /// Data captured from import file
    var v1ImportData: ExportVersionOne? = nil
    /// Selected V1 addresses for import
    var selectedV1Addresses: Set<ExportVersionOneAddress> = []
    /// Dictionary of errors after import attempt [messageId: errorMessage]
    var errorDict: [String: String] = [:]
    /// Data captured from version 2 import file
    var v2ImportData: ExportVersionTwo? = nil
    /// Selected V2 addresses for import
    var selectedV2Addresses: Set<ExportVersionTwoAddress> = []
    /// Version of the import data, application logic will depend on this
    ...
    var selectedExportAddresses: Set<Address> = []
    var selectedExportType: ExportTypes = .encoded

    var textFileDocument = TextFileDocument(text: "Hello World!")
    var isExportingTextFile: Bool = false
    var jsonFileDocument = JSONFileDocument.dummyDoc
    var isExportingJSONFile: Bool = false
    var csvFileDocument = CSVFileDocument(csvText: "")
    var isExportingCSVFile: Bool = false
    ...
    var selectedArchivedAddresses: Set<Address> = []
    var showArchAddrDeleteConf: Bool = false

    // MARK: - Folder page properties
    var selectedFolder: Folder? = nil
    var showDeleteFolderConf: Bool = false
