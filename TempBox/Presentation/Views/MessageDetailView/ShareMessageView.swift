...
struct ShareMessageView: View {
    ...
    private func getMessageData() {
        Task {
            if let message = addressStore.selectedMessage, let address = addressStore.selectedAddress {
                messageDetailController.isLoadingMessageData = true
                defer { messageDetailController.isLoadingMessageData = false }

                do {
                    if let safeData: Data = await addressStore.downloadMessageResource(message: message, address: address) {
    ...
    private func getFileNameFrom(ext: String) -> String {
        if let message = addressStore.selectedMessage, !message.subject.isEmpty {
